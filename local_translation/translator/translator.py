...
from chunker.sentence_based_chunker import SentenceBasedChunker
...
翻訳:"""
    ...
    def translate_text_chunked(self, text: str, target_language: str = "日本語", max_chunk_size: int = 2000, abbreviation_file: str = None) -> str:
        """
        長文を文単位でチャンク分割し、各チャンクごとに翻訳して結合する
        Args:
            text: 翻訳対象のテキスト
            target_language: 翻訳先言語
        ...
        if abbreviation_file:
            chunker = SentenceBasedChunker(max_chunk_size=max_chunk_size, abbreviation_file=abbreviation_file)
        else:
            chunker = SentenceBasedChunker(max_chunk_size=max_chunk_size)
        chunks = chunker.sentence_based_chunk(text)
        translated_chunks = []
        for idx, chunk in enumerate(chunks):
            ...
            try:
                ...
                print(f"チャンク{idx+1}の翻訳エラー: {e}")
                translated_chunks.append(chunk)  # エラー時は原文を返す
